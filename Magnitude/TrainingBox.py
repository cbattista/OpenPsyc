...
while len(stimList):
	...
	elif condition == "m" and stim == "mag":
		#Make Rectangles
		x = screen.size[0] / 4
		y = screen.size[1] / 2
		problem = "%s or %s" % (n1, n2)

		box1 = TextureStimulus(color = [1, 1, 1], anchor = 'center', position = [x, y], size=[boxsize, boxsize])
		box2 = TextureStimulus(color = [1, 1, 1], anchor = 'center', position = [x * 3, y], size= [boxsize, boxsize])
		ns1 = Text(text = str(n1), anchor = 'center', position = [x, y], color = [0,0,0], font_size = fontsize)
		ns2 = Text(text = str(n2), anchor = 'center', position = [x * 3, y], color = [0,0,0], font_size = fontsize)
